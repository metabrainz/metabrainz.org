...
class OAuthTestCase(TestCase):
    ...
    def test_oauth_token_refresh_success(self):
        ...
        with patch.object(
            ...
            old_token = db.session.query(OAuth2Token).filter_by(access_token=token["access_token"]).first()
    ...
    def _test_oauth_token_error_helper(self, access_token, data, error):
        with patch.object(RefreshTokenGrant, "authenticate_user", return_value=self.user2):
            ...
            old_token = db.session.query(OAuth2Token).filter_by(access_token=access_token).first()
